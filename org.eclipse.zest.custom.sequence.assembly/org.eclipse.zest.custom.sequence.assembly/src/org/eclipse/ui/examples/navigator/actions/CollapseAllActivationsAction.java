...
public class CollapseAllActivationsAction extends Action {

	private Activation activation;
	private UMLSequenceViewer viewer;
	
	public CollapseAllActivationsAction(UMLSequenceViewer viewer) {
		this.viewer = viewer;
	}
		...
		//try {
			...
			viewer.getChart().getDisplay().asyncExec(new Runnable(){
				...
				public void run() {
					...
					LinkedList<Activation> activations = new LinkedList<Activation>();
					monitor.beginTask("Collapsing Activations", IProgressMonitor.UNKNOWN);
					int total = 0;
					activations.add(activation);
					try {
						activation.getChart().setRedraw(false);
