 ...
 * @since 3.2
 */
public class OpenFunctionAction extends ActionDelegate {
			 ...
			 if(selection.size() == 1) {
				 ...
				 if(stuff.indexOf("[module: ") < 0){				 
					 ...
					 if(test != null && !test.isLinked()){
						 ...
						 try {
							 AssemblySequenceEditor editor = (AssemblySequenceEditor) IDE.openEditor(
						 ...
						 try {
							 AssemblySequenceEditor editor = (AssemblySequenceEditor) page.openEditor(new FileStoreEditorInput(fileStore), "org.eclipse.zest.custom.sequence.assembly.editors.AssemblySequenceEditor");
							 Activator.getDefault().editors.add(editor);
						} catch (PartInitException e) {
